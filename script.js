...
function getWebGLContext (canvas) {
    ...
    // Fixed: Added safety check for Google Analytics
...
} // FIXED: Closed startGUI function
...
function createTextureAsync (url) {
    ...
    let image = new Image();
    image.onload = () => { obj.width = image.width; obj.height = image.height; gl.bindTexture(gl.TEXTURE_2D, texture); gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGB, gl.RGB, gl.UNSIGNED_BYTE, image); };
    image.src = url;
    return obj;
}
...
function wrap (v, min, max) { let range = max - min; return range == 0 ? min : (v - min) % range + min; }
